...
def loc():
    ...
    folium_map = folium.Map(zoom_start=17,tiles="Stamen Terrain",world_copy_jump=True)
    folium.Marker(start_coords, popup="<i>International Space Station</i>", tooltip=tooltip,icon=folium.Icon(color="red", icon="info-sign")).add_to(folium_map)
    #points=(0,0)
    folium.CircleMarker(start_coords,radius=50,fill=True,color='red').add_to(folium_map)
